...
const RozgrywkiPage = ({ data }) => (
  <Layout>
    <Seo title={data.wpPage.ustawienia.ustawieniaTytulStrony} />
    <Container>
      <SectionTitle className="top-overlay" title="Rozgrywki" p="145px 0 8px 0" pMD="320px 0 8px 0" mb="48px" />
      {data.allWpPage.nodes[0].content ? <Content>{parse(data.allWpPage.nodes[0].content)}</Content> : <Text>Strona w trakcie budowy</Text>}
    </Container>
  </Layout>
)

export default RozgrywkiPage

export const pageQuery = graphql`
  query WordpressRozgrywki {
    ...
    allWpPage(filter: {id: {eq: "cG9zdDo4MjI="}}) {
      nodes {
        title
        content
      }
